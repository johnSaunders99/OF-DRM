...
namespace OF_DRM_Video_Downloader.Helpers
{
    public class DBHelper : IDBHelper
    {
            ...
            try
            {
                ...
                using (SqliteConnection connection = new SqliteConnection($"Data Source={dbFilePath}"))
                {
                    ...
                    using (SqliteCommand cmd = new SqliteCommand(
                               "CREATE TABLE IF NOT EXISTS medias (id INTEGER NOT NULL, media_id INTEGER, post_id INTEGER NOT NULL, link VARCHAR, directory VARCHAR, filename VARCHAR, size INTEGER, api_type VARCHAR, media_type VARCHAR, preview INTEGER, linked VARCHAR, downloaded INTEGER, created_at TIMESTAMP, PRIMARY KEY(id), UNIQUE(media_id));",
                               connection))
                    {
                        cmd.ExecuteNonQuery();
                    }
                    ...
                    using (SqliteCommand cmd = new SqliteCommand(
                               "CREATE TABLE IF NOT EXISTS messages (id INTEGER NOT NULL, post_id INTEGER NOT NULL, text VARCHAR, price INTEGER, paid INTEGER, archived BOOLEAN, created_at TIMESTAMP, user_id INTEGER, PRIMARY KEY(id), UNIQUE(post_id));",
                               connection))
                    {
                        cmd.ExecuteNonQuery();
                    }
                    ...
                    using (SqliteCommand cmd = new SqliteCommand(
                               "CREATE TABLE IF NOT EXISTS posts (id INTEGER NOT NULL, post_id INTEGER NOT NULL, text VARCHAR, price INTEGER, paid INTEGER, archived BOOLEAN, created_at TIMESTAMP, PRIMARY KEY(id), UNIQUE(post_id));",
                               connection))
                    {
                        cmd.ExecuteNonQuery();
                    }
                    ...
                    using (SqliteCommand cmd = new SqliteCommand(
                               "CREATE TABLE IF NOT EXISTS stories (id INTEGER NOT NULL, post_id INTEGER NOT NULL, text VARCHAR, price INTEGER, paid INTEGER, archived BOOLEAN, created_at TIMESTAMP, PRIMARY KEY(id), UNIQUE(post_id));",
                               connection))
                    {
                        cmd.ExecuteNonQuery();
                    }
                    ...
                    using (SqliteCommand cmd = new SqliteCommand(
                               "CREATE TABLE IF NOT EXISTS others (id INTEGER NOT NULL, post_id INTEGER NOT NULL, text VARCHAR, price INTEGER, paid INTEGER, archived BOOLEAN, created_at TIMESTAMP, PRIMARY KEY(id), UNIQUE(post_id));",
                               connection))
                    {
                        cmd.ExecuteNonQuery();
                    }
                    ...
                    using (SqliteCommand cmd = new SqliteCommand(
                               "CREATE TABLE IF NOT EXISTS products (id INTEGER NOT NULL, post_id INTEGER NOT NULL, text VARCHAR, price INTEGER, paid INTEGER, archived BOOLEAN, created_at TIMESTAMP, title VARCHAR, PRIMARY KEY(id), UNIQUE(post_id));",
                               connection))
                    {
                        cmd.ExecuteNonQuery();
                    }
                    ...
                    using (SqliteCommand cmd = new SqliteCommand(
                               "CREATE TABLE IF NOT EXISTS profiles (id INTEGER NOT NULL, user_id INTEGER NOT NULL, username VARCHAR NOT NULL, PRIMARY KEY(id), UNIQUE(username));",
                               connection))
                    {
                        cmd.ExecuteNonQuery();
                    }
        ...
        public async Task AddMessage(string folder, long post_id, string message_text, string price, bool is_paid,
            bool is_archived, DateTime created_at, int user_id)
        {
            try
            {
                using (SqliteConnection connection =
                       new SqliteConnection($"Data Source={folder}/Metadata/user_data.db"))
                {
                    connection.Open();
                    using (SqliteCommand cmd =
                           new SqliteCommand($"SELECT COUNT(*) FROM messages WHERE post_id=@post_id", connection))
                    {
                        ...
                        if (count == 0)
                        {
                            ...
                            using (SqliteCommand insertCmd = new SqliteCommand(
                                       "INSERT INTO messages(post_id, text, price, paid, archived, created_at, user_id) VALUES(@post_id, @message_text, @price, @is_paid, @is_archived, @created_at, @user_id)",
                                       connection))
                            {
                                insertCmd.Parameters.AddWithValue("@post_id", post_id);
                                insertCmd.Parameters.AddWithValue("@message_text",
                                    message_text ?? (object)DBNull.Value);
            ...
            bool is_archived, DateTime created_at)
        {
            try
            {
                ...
                {
                    connection.Open();
                    using (SqliteCommand cmd = new SqliteCommand($"SELECT COUNT(*) FROM posts WHERE post_id=@post_id",
                               connection))
                    {
                        cmd.Parameters.AddWithValue("@post_id", post_id);
                        int count = Convert.ToInt32(cmd.ExecuteScalar());
                        if (count == 0)
                        {
                            // If the record doesn't exist, insert a new one
                            using (SqliteCommand insertCmd = new SqliteCommand(
                                       "INSERT INTO posts(post_id, text, price, paid, archived, created_at) VALUES(@post_id, @message_text, @price, @is_paid, @is_archived, @created_at)",
                                       connection))
                            {
                                insertCmd.Parameters.AddWithValue("@post_id", post_id);
                                insertCmd.Parameters.AddWithValue("@message_text",
            ...
            DateTime? created_at)
        {
            try
            {
                       ...
                       new SqliteConnection($"Data Source={folder}/Metadata/user_data.db"))
                {
                    connection.Open();
                    using (SqliteCommand cmd =
                           new SqliteCommand($"SELECT COUNT(*) FROM medias WHERE media_id={media_id}", connection))
                    {
                        int count = Convert.ToInt32(cmd.ExecuteScalar());
                        if (count == 0)
                        {
        ...
        public async Task<bool> CheckDownloaded(string folder, long media_id, string type, string file_name)
        {
            try
            {
                       ...
                       new SqliteConnection($"Data Source={folder}/Metadata/user_data.db"))
                {
                    connection.Open();
                    using (SqliteCommand cmd =
                           new SqliteCommand($"SELECT downloaded FROM medias WHERE media_id={media_id}", connection))
                    {
                        downloaded = Convert.ToBoolean(cmd.ExecuteScalar());
                    }
                }

                string targetDir = Path.Combine(folder, type.TrimStart('/'));
                string outputPath = Path.Combine(targetDir, file_name);

                return downloaded && File.Exists(outputPath);
            }
            catch (Exception ex)
            {
                Console.WriteLine("Exception caught: {0}\n\nStackTrace: {1}", ex.Message, ex.StackTrace);
        ...
        public async Task UpdateMedia(string folder, long media_id, string directory, string filename, long size,
